...
**And a rule of thumb is closer to real practice than this model is.** Scored on these same decisions and this same metric, a threshold rule using only the fitted slope and the measured pit loss lands a median of 7 laps from the real stop against the optimiser's 10, and is within two laps more often. See [`../cross_series/baselines.md`](../cross_series/baselines.md). That does not make the rule *better strategy* — every number here scores agreement with what teams did, not what was fastest — but it removes the explanation that the gap comes from information the simpler methods lack.
